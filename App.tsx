...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 1. Intersection Observer for section detection
    const observerOptions = {
      root: null,
      rootMargin: '-20% 0px -70% 0px', // Detect when a section starts taking up the upper portion of screen
      threshold: 0
    };
    ...
    // 2. Window scroll fallback for the "Top" (Home) area
    const handleScrollFallback = () => {
      if (isNavigatingRef.current) return;
      // If we are at the very top, force HOME state
  ...
  const handleNavigate = (view: AppView) => {
    if (isLandingView(view)) {
      ...
      if (isLandingView(currentView)) {
        ...
        // Return from a tool view to the landing stack
        setCurrentView(view);
        setTimeout(() => {
      ...
      // Navigate to specialized AI tool view
      setCurrentView(view);
