...
const NRPHub: React.FC<NRPHubProps> = ({ onLaunchTool }) => {
  ...
  return (
    <div className="bg-brand-charcoal min-h-screen pb-56 text-white">
      {/* 1. HERO HEADER */}
      ...
      <div className="max-w-[1700px] mx-auto px-8 lg:px-16 mt-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {tools.map((tool, idx) => (
            <div 
              key={idx} 
              className="bg-[#262626] border border-white/5 p-12 rounded-[3.5rem] flex flex-col justify-between h-[500px] hover:bg-[#333333] hover:border-brand-gold transition-all cursor-pointer group shadow-2xl"
              onClick={() => onLaunchTool(tool.id)}
            >
              <div className="space-y-10">
                <div className="w-16 h-16 bg-brand-gold text-black rounded-3xl flex items-center justify-center shadow-lg shadow-brand-gold/10 group-hover:rotate-6 transition-transform group-hover:bg-white">
                  <tool.icon size={32} />
                </div>
                <div>
                  <h3 className="text-2xl brand-text-heavy uppercase tracking-tight mb-4 text-white group-hover:text-brand-gold transition-colors">{tool.name}</h3>
                  <p className="text-[13px] brand-text-heavy text-slate-400 uppercase tracking-widest opacity-80 leading-relaxed group-hover:text-white transition-colors">
                    {tool.desc}
                  </p>
                </div>
              </div>
              
              <div className="flex items-center gap-4 group-hover:gap-6 transition-all bg-[#1a1a1a] w-fit px-8 py-4 rounded-2xl border border-white/5 group-hover:border-brand-gold/40">
                <span className="text-[13px] brand-text-heavy uppercase tracking-[0.3em] text-brand-gold group-hover:text-white transition-colors">LAUNCH MODULE</span>
